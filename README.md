...
FastAPI backend for VendrSurf. Hosts Anthropic parse, Crust Data vendor discovery, and (future) phone-call webhook handlers.

## Endpoints

- `POST /parse-rfq` — `{text}` → `{fields}` (location ISO3, product_category, quantity, budget_min, budget_max, timeline_weeks)
